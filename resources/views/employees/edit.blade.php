                    ...
                    <form action="{{ route('employees.update', $employee) }}" method="POST">
                        ...
                        @method('PUT') <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="name" id="name" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('name', $employee->name) }}" required>
                            </div>
                            <div>
                                <label for="cpf" class="block font-medium text-sm text-gray-700">CPF</label>
                                <input x-init="IMask($el, cpfMaskOptions)" type="text" name="cpf" id="cpf" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('cpf', $employee->cpf) }}" required>
                            </div>
                            <div>
                                <label for="birth_date" class="block font-medium text-sm text-gray-700">Data de Nascimento</label>
                                <input type="date" name="birth_date" id="birth_date" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('birth_date', $employee->birth_date->format('Y-m-d')) }}" required>
                            </div>
                            <div>
                                <label for="phone" class="block font-medium text-sm text-gray-700">Telefone</label>
                                <input x-init="IMask($el, phoneMaskOptions)" type="tel" name="phone" id="phone" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('phone', $employee->phone) }}" required>
                            </div>
                            <div class="md:col-span-2">
                                <label for="gender" class="block font-medium text-sm text-gray-700">Gênero</label>
                                <select name="gender" id="gender" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" required>
